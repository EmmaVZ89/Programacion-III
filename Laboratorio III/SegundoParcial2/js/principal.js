    ...
    $.ajax({
        ...
        if (resultado.exito) {
            ...
            $('[data-action="modificar-auto"]').on('click', function (e) {
                let obj_auto_string = $(this).attr("data-obj_auto");
            ...
            let alerta = ArmarAlert(resultado.mensaje, "danger");
            $("#divResultado").html(alerta).show(2000);
        }
    })
        .fail(function (jqXHR, textStatus, errorThrown) {
...
function MostrarFormAuto(accion, obj_auto = null) {
    let funcion = "";
    let encabezado = "";
    let solo_lectura = "";
    let solo_lectura_pk = "";
    ...
    $.ajax({
        ...
        if (resultado.exito) {
            ...
            $('[data-action="modificar-usuario"]').on('click', function (e) {
                let obj_user_string = $(this).attr("data-obj_user");
...
function MostrarFormUsuario(accion, obj_user = null) {
    let funcion = "";
    let encabezado = "";
    let solo_lectura = "";
    let solo_lectura_pk = "";
...
function AltaAuto() {
    ...
    $("#btnAgregar").on("click", function (e) {
        ...
        $.ajax({
            ...
            .done(function (resultado) {
            let mensaje = "Usuario válido!";
            let tipo = "success";
            console.log(resultado.mensaje);
            if (resultado.exito) {
                // e.preventDefault();
                mensaje = resultado.mensaje;
                let alerta = ArmarAlert(mensaje, tipo);
                $("#divResultado").html(alerta).toggle(2000);
            }
            else {
                mensaje = resultado.mensaje;
...
function FiltrarAutosPrecioColor() {
    ...
    $.ajax({
        ...
        let filtrado = resultado.dato.filter((dato) => dato.precio > 199999 && dato.color != "rojo");
...
function PrecioPromedioAutos() {
    ...
    $.ajax({
        ...
        .done(function (resultado) {
        let mapeado = resultado.dato.filter(function (auto) {
            if (auto.marca[0] == "F" || auto.marca[0] == "f") {
                return { precio: auto.precio };
            }
        });
        let prom = mapeado.reduce((a, x) => parseInt(a) + parseInt(x.precio), 0) / mapeado.length;

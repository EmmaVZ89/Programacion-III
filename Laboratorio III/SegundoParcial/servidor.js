...
// Listar usuarios
app.get('/usuarios', verificar_jwt, (request, response) => {
    let obj_respuesta = {
        "exito": false,
...
// CRUD AUTOS **************************************************************************************
// Agregar auto
app.post('/autos', alta_baja, (request, response) => {
    let obj_respuesta = {
        "exito": false,
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
        conn.query("INSERT INTO autos set ?", [auto], (err, rows) => {
            if (err) {
                console.log(err);
                throw ("Error en consulta de base de datos.");
            }
...
// Listar autos
app.get('/autos', verificar_jwt, (request, response) => {
    let obj_respuesta = {
        "exito": false,
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
        conn.query("SELECT * FROM autos", (err, rows) => {
            if (err)
                throw ("Error en consulta de base de datos.");
            if (rows.length == 0) {
                response.status(obj_respuesta.status).json(obj_respuesta);
...
// Modificar auto
app.put('/autos/modificar', modificar, (request, response) => {
    let obj_respuesta = {
        "exito": false,
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
        conn.query("UPDATE autos set ?  WHERE id = ?", [auto_modif, auto.id], (err, rows) => {
            if (err) {
                console.log(err);
                throw ("Error en consulta de base de datos.");
            }
...
// Eliminar auto
app.delete('/autos/eliminar', alta_baja, (request, response) => {
    let obj_respuesta = {
        "exito": false,
    ...
    request.getConnection((err, conn) => {
        if (err)
            throw ("Error al conectarse a la base de datos.");
        conn.query("DELETE FROM autos WHERE id = ?", [obj.id], (err, rows) => {
            if (err) {
                console.log(err);
                throw ("Error en consulta de base de datos.");
            }

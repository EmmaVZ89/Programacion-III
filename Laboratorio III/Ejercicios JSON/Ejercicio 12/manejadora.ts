namespace Entidades {
  ...
  export function LeerArchivoJsonCiudades3(): void {
      ...
      if (xhttp.readyState === DONE) {
        if (xhttp.status === OK) {
          ...
          ciudadesJson.forEach((ciudad: any) => {
            ...
            a.setAttribute(
              "onclick",
              `Entidades.EliminarCiudad(${ciudad["_id"]})`
            );
  ...
  export function AgregarCiudad2(): void {
    ...
    let pais: string = (<HTMLInputElement>document.getElementById("txtPais"))
      .value;
    let longitud: number = parseInt(
      (<HTMLInputElement>document.getElementById("txtLongitud")).value
    );
    let latitud: number = parseInt(
      (<HTMLInputElement>document.getElementById("txtLatitud")).value
    );
    let ciudad: any = {
      _id: id,
      name: nombre,
  ...
  export function EliminarCiudad(id: number): void {
    xhttp.open("POST", "./backend/administrarCiudades.php", true);

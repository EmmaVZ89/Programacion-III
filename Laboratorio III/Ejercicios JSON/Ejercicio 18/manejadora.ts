namespace Entidades {
  ...
  export function CargarRemeras5(): void {
      ...
      if (xhttp.readyState === DONE) {
        if (xhttp.status === OK) {
          ...
          remerasJson.forEach((remera: any) => {
            ...
            a.setAttribute(
              "onclick",
              `Entidades.EliminarRemera(${remera["id"]})`
            );
  ...
  export function FiltrarRemerasPorCampo3(): void {
  ...
  export function AgregarRemera2(): void {
  ...
  export function EliminarRemera(idRemera : number) : void {
    let respuesta : boolean =  confirm(`¿ Desea eliminar articulo con ID "${idRemera}" ?`);
    if(respuesta) {
      xhttp.open("POST", "./backend/administrarRemeras.php", true);

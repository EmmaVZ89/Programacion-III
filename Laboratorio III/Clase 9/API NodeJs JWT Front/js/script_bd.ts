    ...
    else
    {
        productos.forEach((prod : any) => {
            ...
            for (const key in prod) {
                if(key != "path") {
                    tabla += "<td>"+prod[key]+"</td>";
                } else {
                    tabla += "<td><img src='"+URL_API+prod.path+"' width='50px' height='50px'></td>";
                }
            }
            tabla += "<td><a href='#' class='btn' data-action='modificar' data-obj_prod='"+JSON.stringify(prod)+"' title='Modificar'"+
            " data-toggle='modal' data-target='#ventana_modal_prod' ><span class='fas fa-edit'></span></a>";
            tabla += "<a href='#' class='btn' data-action='eliminar' data-obj_prod='"+JSON.stringify(prod)+"' title='Eliminar'"+
